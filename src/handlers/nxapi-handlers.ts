...
export async function handleProxy(body: {
    ...
    const defaultUserAgent = allowedNxapiHosts.has(targetHost)
        ...
        : 'com.nintendo.znca/3.5.0 (Android/10)';
    ...
    if (contentLength > 20 * 1024 * 1024 || responseContentType.startsWith('video/') || responseContentType.startsWith('audio/')) {
        return {
            status: 413,
            data: { error: 'payload_too_large', message: 'Direct browser fetch should be used for large media payloads (> 20 MiB).' }
        };
    }
    ...
    try {
        ...
        return { status: response.status, data: null, text: textData };
    }
}
